...
func _ready() -> void:
	...
	match last_movement:
		Vector2.UP, Vector2.DOWN:
			move_to_less = global_position + Vector2(randf_range(-1, -0.125), last_movement.y) * 500
			move_to_more = global_position + Vector2(randf_range(0.125, 1), last_movement.y) * 500
